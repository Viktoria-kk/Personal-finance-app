(function () {
  ...
  function openModal(id, trigger) {
    ...
    if (!m) return;
    if (id === "modal-add-money" || id === "modal-withdraw")
      setupMoney(m, trigger);
    if (id === "modal-delete-budget" || id === "modal-delete-pot") {
      var t = m.querySelector("[data-del-title]");
      if (t && trigger && trigger.dataset.name)
        t.textContent = "Delete ‘" + trigger.dataset.name + "’?";
    }
  ...
  function setupMoney(m, trigger) {
    ...
    m.querySelector("[data-money-target]").textContent =
  ...
  function updateMoney(m) {
    ...
    var next =
      mode === "add" ? m._money.saved + amt : Math.max(m._money.saved - amt, 0);
    ...
    if (minimize) {
      ...
      if (sb) {
        ...
        localStorage.setItem(
          "sidebar-minimized",
          sb.classList.contains("is-minimized"),
        );
      }
      return;
    }
